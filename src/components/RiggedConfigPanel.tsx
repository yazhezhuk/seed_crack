...
const THEMES: { id: string; label: string; swatch: string }[] = [
  { id: "green", label: "GREEN", swatch: "hsl(130 100% 55%)" },
  { id: "red", label: "RED", swatch: "hsl(0 100% 55%)" },
  { id: "amber", label: "AMBER", swatch: "hsl(38 100% 55%)" },
  { id: "cyan", label: "CYAN", swatch: "hsl(190 100% 55%)" },
  { id: "purple", label: "PURPLE", swatch: "hsl(280 100% 65%)" },
];
...
export const RiggedConfigPanel = ({
  ...
  const addHit = () => {
    ...
    const rawBalance = draft.balance.trim();
    const rawLabel = draft.balanceLabel.trim();
    const balance =
        Number(rawBalance.replace(/[^0-9.,|-]/g, "").replace(/[,|]/g, ".")) || 0;
    ...
    const next: RiggedHit = {
      ...
      balance,
      balanceLabel: rawLabel || rawBalance || undefined,
    };
    setHits(
              ...
              {THEMES.map((t) => {
                ...
                return (
                    <button
                        ...
                        className={`flex items-center gap-1.5 border-2 px-2 py-1 text-[10px] font-bold uppercase tracking-widest transition-all active:scale-95 ${
                            on
          ...
          <div className="mb-3 border border-border/60 bg-card/60 p-3 space-y-2">
            ...
            <div className="flex gap-2">
              <input
                  ...
                  placeholder="balance number (e.g. 2.45)"
            ...
            <div className="flex gap-2">
              <input
                  ...
                  onChange={(e) => setDraft({ ...draft, balanceLabel: e.target.value })}
                  placeholder="displayed balance text (e.g. $2.45 | 0.5 BTC)"
                  className="flex-1 border-2 border-border bg-background px-2 py-1.5 text-xs font-bold text-primary focus:border-primary focus:outline-none"
              />
              <button
                  type="button"
                  onClick={addHit}
